  ...
  return (
    <div className="page-container space-y-6 md:space-y-8">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-4xl lg:text-5xl font-extrabold text-text-primary tracking-tight">
          ...
          <p className="subheading mt-1 md:mt-3 max-w-2xl text-xs md:text-base">
        ...
        <div className="flex gap-2 md:gap-3 rtl:flex-row-reverse">
          {members.slice(0, 5).map((member) => (
            <div 
              key={member.userId} 
              className="h-10 w-10 md:h-12 md:w-12 rounded-full border-2 md:border-4 border-background bg-primary text-white flex items-center justify-center font-bold text-sm md:text-lg shadow-sm overflow-hidden"
          ...
          {members.length > 5 && (
            <div className="h-10 w-10 md:h-12 md:w-12 rounded-full border-2 md:border-4 border-background bg-gray-200 text-text-secondary flex items-center justify-center font-bold text-sm md:text-base shadow-sm">
              +{members.length - 5}
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8">
        {/* Bills Card */}
        <div className="bg-white rounded-2xl md:rounded-3xl p-5 md:p-8 shadow-sm border border-gray-100 flex flex-col transform transition-transform hover:scale-[1.01]">
          <div className="flex justify-between items-start mb-4 md:mb-6">
            <div className="p-2 md:p-3 bg-warning-light rounded-xl md:rounded-2xl border border-warning-light">
          ...
          <p className="text-text-secondary text-[10px] md:text-sm mb-4 md:mb-8">
        ...
        <div className="bg-white rounded-2xl md:rounded-3xl p-5 md:p-8 shadow-sm border border-gray-100 flex flex-col transform transition-transform hover:scale-[1.01]">
          <div className="flex justify-between items-start mb-4 md:mb-6">
            <div className="p-2 md:p-3 bg-info-light rounded-xl md:rounded-2xl border border-info-light">
              <CheckSquare className="h-5 w-5 md:h-8 md:w-8 text-info-dark" />
            </div>
          ...
          <h3 className="text-base md:text-xl font-semibold mb-1 text-text-primary">{t('dashboard.choresTitle')}</h3>
          <p className="text-text-secondary text-[10px] md:text-sm mb-4 md:mb-6">{t('dashboard.nextUp')}: <span className="font-bold text-text-primary truncate inline-block align-bottom max-w-[150px]">{nextUpText}</span></p>
          ...
          <div className="space-y-2.5 md:space-y-3 mb-4 md:mb-6 flex-1">
            {pendingChores.slice(0, 3).map(chore => {
              ...
              return (
                <div 
                  ...
                  className={`flex justify-between items-center text-xs md:text-sm p-2.5 md:p-3 rounded-xl border cursor-pointer transition-all duration-500 ${
                    isCompleting
                ...
                >
                  <div className="flex items-center gap-2.5 md:gap-3 overflow-hidden">
                    {isCompleting ? (
                      <CheckCircle2 className="flex-shrink-0 h-5 w-5 md:h-6 md:w-6 text-info-dark transition-all duration-500 scale-110" />
                    ) : (
                      <Circle className={`flex-shrink-0 h-5 w-5 md:h-6 md:w-6 transition-colors opacity-40 hover:opacity-100 ${isOverdue ? 'text-red-500' : 'text-info-dark'}`} />
                    )}
                    <div className="truncate">
                      ...
                      <p className={`text-[10px] md:text-xs transition-all duration-500 ${isCompleting ? 'text-gray-400' : isOverdue ? 'text-red-600 font-bold' : 'text-text-secondary'}`}>
                  ...
                  {assignedMember && (
                    <div className={`flex-shrink-0 h-5 w-5 md:h-6 md:w-6 rounded-full bg-primary text-white flex items-center justify-center text-[8px] md:text-[10px] font-bold overflow-hidden ${i18n.language === 'ar' ? 'mr-2' : 'ml-2'} transition-opacity duration-500 ${isCompleting ? 'opacity-40' : 'opacity-100'}`} title={assignedMember.user?.fullName}>
            ...
            {pendingChores.length === 0 && (
              <div className="text-xs md:text-sm text-success font-bold text-center py-4">{t('dashboard.allCaughtUp')}</div>
            )}
          </div>

          <Link to="/chores" className="w-full block text-center bg-gray-50 text-text-primary hover:bg-gray-100 font-bold py-2.5 md:py-3 rounded-xl md:rounded-2xl text-xs md:text-sm transition-all shadow-sm active:scale-95 mt-auto uppercase">
        ...
        <div className="bg-white rounded-2xl md:rounded-3xl p-5 md:p-8 shadow-sm border border-gray-100 flex flex-col transform transition-transform hover:scale-[1.01]">
          <div className="flex justify-between items-start mb-4 md:mb-6">
            <div className="p-2 md:p-3 bg-success-light rounded-xl md:rounded-2xl border border-success-light">
              <ShoppingCart className="h-5 w-5 md:h-8 md:w-8 text-success-dark" />
            </div>
          ...
          <h3 className="text-base md:text-xl font-semibold mb-1 text-text-primary">{t('dashboard.groceriesTitle')}</h3>
          <p className="text-text-secondary text-[10px] md:text-sm mb-4 md:mb-6">{neededGroceries.length} {t('dashboard.itemsNeeded')}</p>
          ...
          <ul className="space-y-2.5 md:space-y-3 mb-4 md:mb-6 flex-1">
            {neededGroceries.slice(0, 3).map(item => {
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2.5 md:gap-3 overflow-hidden">
                    {isCompleting ? (
                      <CheckCircle2 className="flex-shrink-0 h-5 w-5 md:h-6 md:w-6 text-success-dark transition-all duration-500 scale-110" />
                    ) : (
                      <Circle className="flex-shrink-0 h-5 w-5 md:h-6 md:w-6 text-success-dark opacity-40 hover:opacity-100 transition-opacity" />
                    )}
                    <div className="truncate">
                      <p className={`font-medium truncate transition-all duration-500 ${isCompleting ? 'text-gray-400 line-through' : 'text-text-primary'}`}>
                        {item.name}
                      </p>
                      <p className={`text-[10px] md:text-xs transition-all duration-500 ${isCompleting ? 'text-gray-400' : 'text-text-secondary'}`}>
                        {t('dashboard.qty')}: {item.quantity || 1}
                      </p>
                    </div>
                  </div>
                  {addedByMember && (
                    <div className={`flex-shrink-0 h-5 w-5 md:h-6 md:w-6 rounded-full bg-success text-white flex items-center justify-center text-[8px] md:text-[10px] font-bold overflow-hidden ${i18n.language === 'ar' ? 'mr-2' : 'ml-2'} transition-opacity duration-500 ${isCompleting ? 'opacity-40' : 'opacity-100'}`} title={addedByMember.user?.fullName}>
            ...
            {neededGroceries.length === 0 && (
              <li className="text-xs md:text-sm text-success-dark font-medium italic text-center py-4">{t('dashboard.fridgeFull')}</li>
            )}
          </ul>
          <Link to="/groceries" className="w-full block text-center bg-primary text-white hover:bg-primary-dark font-bold py-2.5 md:py-3 rounded-xl md:rounded-2xl text-xs md:text-sm transition-all shadow-lg active:scale-95 uppercase">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8">
        {/* Schedule */}
        <div className="md:col-span-2 lg:col-span-3 bg-white rounded-2xl md:rounded-3xl p-5 md:p-8 shadow-sm border border-gray-100">
          <div className="flex justify-between items-center mb-4 md:mb-8">
            <h3 className="text-base md:text-xl font-semibold text-text-primary">{t('dashboard.upcomingEvents')}</h3>
            <Link to="/calendar" className="text-[10px] md:text-sm font-bold text-primary hover:text-primary-dark transition-colors flex items-center uppercase">
              {t('dashboard.fullCalendar')}
              <CalendarDays className={`${i18n.language === 'ar' ? 'mr-2' : 'ml-2'} h-3 w-3 md:h-4 md:w-4`} />
            </Link>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
            {events.slice(0, 3).map(event => (
              <div key={event.id} className="group cursor-pointer bg-gray-50 p-3 md:p-4 rounded-xl border border-gray-100 hover:border-primary/30 transition-colors">
                <div className="text-[10px] md:text-xs font-bold text-primary uppercase tracking-widest mb-1.5 md:mb-2 bg-primary/10 inline-block px-2 py-1 rounded">
                  {new Date(event.startDatetime).toLocaleDateString(i18n.language === 'ar' ? 'ar-EG' : 'en-US', { weekday: 'short', day: 'numeric', month: 'short' })}
                </div>
                <h4 className="font-semibold text-text-primary text-sm md:text-base group-hover:text-primary transition-colors truncate">{event.title}</h4>
                <p className="text-xs md:text-sm text-text-secondary flex items-center mt-1">
                  <span className={`w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-primary ${i18n.language === 'ar' ? 'ml-2' : 'mr-2'}`}></span>
                  {new Date(event.startDatetime).toLocaleTimeString(i18n.language === 'ar' ? 'ar-EG' : 'en-US', { hour: '2-digit', minute: '2-digit' })}
                </p>
              </div>
            ))}
            {events.length === 0 && (
              <div className="col-span-1 md:col-span-3 py-8 md:py-12">
                <EmptyState 
                  icon={CalendarDays} 
