...
import { format, parseISO, differenceInSeconds, isBefore, isAfter } from 'date-fns';
...
export default function TVShowerQueueWidget({ apartmentId }: { apartmentId: string }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      // Filter for 'now' mode slots only for upcoming, but always show active slots
      const filteredSlots = fetchedSlots.filter(s => s.status === 'active' || s.mode === 'now');
  ...
  const activeSlot = slots.find(s => s.status === 'active' || (s.status === 'scheduled' && isBefore(parseISO(s.startTime), now) && isAfter(parseISO(s.endTime), now)));
  const upNext = slots.filter(s => s.status === 'scheduled' && (!activeSlot || s.id !== activeSlot.id));
        ...
        <div className="mt-auto pt-6 border-t border-slate-800/50">
          ...
          <div className="flex justify-between items-center bg-slate-800/30 p-5 rounded-2xl border border-slate-800">
            ...
            <span className="text-xl text-emerald-400 font-mono font-bold">
              {format(parseISO(nextPerson.startTime), 'HH:mm')}
            </span>
          </div>
        </div>
      )}
      ...
      {remainingQueue.length > 0 && (
        <div className="mt-4 pt-4 border-t border-slate-800/30">
          ...
          <ul className="space-y-2">
            {remainingQueue.slice(0, 2).map(slot => (
              <li key={slot.id} className="flex justify-between items-center opacity-40">
                ...
                <span className="text-base text-slate-400 font-mono">{format(parseISO(slot.startTime), 'HH:mm')}</span>
              </li>
            ))}
          </ul>
        </div>
